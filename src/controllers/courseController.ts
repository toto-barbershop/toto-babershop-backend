...
export const getCourse = async (req: Request, res: Response) => {
  try {
    const data = await prisma.course.findUnique({ where: { id: String(req.params.id) } });
...
export const updateCourse = async (req: Request, res: Response) => {
  try {
    const data = await prisma.course.update({ where: { id: String(req.params.id) }, data: req.body });
    res.json(data);
  } catch (error: any) {
