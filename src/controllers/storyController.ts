...
export const getStory = async (req: Request, res: Response) => {
  try {
    const data = await prisma.story.findUnique({ where: { id: String(req.params.id) } });
...
export const updateStory = async (req: Request, res: Response) => {
  try {
    const data = await prisma.story.update({ where: { id: String(req.params.id) }, data: req.body });
    res.json(data);
  } catch (error: any) {
